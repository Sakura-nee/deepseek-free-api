...
async function getIPAddress() {
  ...
  const result = await cycletls.get('https://chat.deepseek.com/', {
    headers: {
      ...FAKE_HEADERS,
  ...
  const ip = Buffer.from(result.data).toString('utf-8').match(/<meta name="ip" content="([\d.]+)">/)?.[1];
...
async function requestToken(refreshToken: string) {
  ...
  const result = await (async () => {
    const result = await cycletls.get(
...
async function createSession(model: string, refreshToken: string): Promise<string> {
  ...
  const result = await cycletls.post(
...
async function getChallengeResponse(refreshToken: string, targetPath: string) {
  const token = await acquireToken(refreshToken);
  const result = await cycletls.post('https://chat.deepseek.com/api/v0/chat/create_pow_challenge', {
    body: JSON.stringify({
      target_path: targetPath
    }),
    headers: {
...
async function createCompletion(
  ...
  return (async () => {
    ...
    if (result.headers["Content-Type"].indexOf("text/event-stream; charset=utf-8") == -1) {
...
async function DeleteConversation(refreshToken: string,
  refConvId?: string,
  retryCount = 0) {
 ...
 * @param refreshToken 用于刷新access_token的refresh_token
 */
function checkResult(result: any, refreshToken: string) {
...
async function receiveStream(model: string, stream: any, refConvId?: string): Promise<any> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const parser = createParser((event) => {
      // console.log(event);
      // fs.appendFileSync("dump.json", JSON.stringify(event) + "\n")
      try {
        if (event.type === "event" && event.data.includes("status") && event.data.includes("accumulated_token_usage")) {
          console.log("Hooked metadata");

          const metadata = _.attempt(() => JSON.parse(event.data));
          if (metadata.v && Array.isArray(metadata.v)) {
            metadata.v.forEach(item => {
              if (item.p && item.p === "accumulated_token_usage") {
        ...
        if (event.type !== "event" || (event.data.trim().includes("FINISHED") && event.data.trim().includes("accumulated_token_usage"))) return;
        // 解析JSON
        ...
        // hard coded
        if (!data.id) data.id = `${refConvId}@${Math.floor(new Date().getTime() / 1000)}`;

        if (event.type === "event" && event.data) {
          if (isJSON(event.data)) {
            if (result?.v?.response?.thinking_enabled && result?.v?.response?.thinking_enabled === true) {
              stillThinking = true;
            }

            else if (result.p && result.p === "response/thinking_content") {
              stillThinking = true;
              if (result.v && typeof result.v !== "object") {
                data.choices[0].message.reasoning_content += result.v;
              }
            }

            else if (stillThinking && !result.p) {
              if (result.v && typeof result.v !== "object") {
                data.choices[0].message.reasoning_content += result.v;
              }
            }

            else if (stillThinking && result.o && result.p && result.p === "response/thinking_elapsed_secs" && result.o === "SET") {
              stillThinking = false;
            }

            else if (result.p && result.o && result.p === "response/content" && result.o === "APPEND") {
              stillThinking = false;
              if (result.v && typeof result.v !== "object") {
            ...
            else if (!stillThinking && result.v) {
              if (result.v && typeof result.v !== "object") {
                data.choices[0].message.content += result.v;
              }
            }
          } else {
            console.log("Unconditional:", result)
          }
        }

        if (event.event && event.event === "finish") {
          resolve(data);
        }
...
async function sendEvents(refConvId: string, refreshToken: string) {
  try {
    ...
    const response = await cycletls.post('https://chat.deepseek.com/api/v0/events', {
      headers: {
        Authorization: `Bearer ${token}`,
        ...FAKE_HEADERS,
...
async function getThinkingQuota(refreshToken: string) {
  try {
    const response = await cycletls.get('https://chat.deepseek.com/api/v0/users/feature_quota', {
      headers: {
        Authorization: `Bearer ${refreshToken}`,
        ...FAKE_HEADERS,
        Cookie: generateCookie()
...
async function fetchAppVersion(): Promise<string> {
  try {
    logger.info('自动获取版本号');
    const response = await cycletls.get('https://chat.deepseek.com/version.txt', {
      timeout: 5000,
      headers: {
        ...FAKE_HEADERS,
        Cookie: generateCookie()
